...
import com.tranex.mobile.Global;
import com.tranex.mobile.dataList;
...
public class DataDB {
	...
	public String getScanOpCode(Context context, String status){
		con = new Connection(context);
		String OpCode = "";
		try {
			con.createDataBase();
		...
		if(con.checkDataBase()){
			con.openDataBase();
			SQLiteDatabase db = con.getWritableDatabase();
			Cursor cursor = db.rawQuery("SELECT * FROM scanoperations WHERE opname='" + status + "'", null);
			...
			if (cursor.getCount() < 1) // UserName Not Exist
			{
				...
				if (cursor.moveToFirst()) {
					OpCode = cursor.getString(cursor.getColumnIndex("opcode"));
